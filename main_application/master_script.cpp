...
void getLogFilePath(char *logFilePath, size_t size) {
    ...
    snprintf(logFilePath, size, "%s/master_log_%04d-%02d-%02d.txt", LOG_DIR, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);
}
...
int main() {
    int currentMode = 17;  // launch main_mode_rotery
    pid_t child_pid = -1;
